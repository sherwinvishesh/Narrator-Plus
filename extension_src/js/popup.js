...
$(function() {
  ...
  else getCurrentTab().then(function(currentTab) {return updateSettings({narratorTab: currentTab.id})})
  ...
  updateButtons()
    .then(getSettings.bind(null, ["showHighlighting", "narratorTab"]))
      ...
      if (settings.showHighlighting == 2 && queryString.isPopup) {
        return getActiveTab()
          .then(function(activeTab) {
            ...
            return (settings.narratorTab ? Promise.resolve() : Promise.reject("No narratorTab"))
              .then(function() {return updateTab(settings.narratorTab, {url: url, active: true})})
